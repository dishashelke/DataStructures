...
public class Sort {
	...
	private static int choosePivote(int low, int hi) {
		// return (low+hi)/2;
		return low;
	}
	...
	private static int quickSelect(int[] array, int low, int hi, int k) {
		if (low == hi)
			return array[low];
		int pivoteIndex = partition(array, low, hi);
		if (pivoteIndex == k)
	...
	// *************************print k smallest elements in array *******************
	public static int[] getKSmallestElements(int array[], int k) {
		return getKSmallestElements(array, 0, array.length - 1, k);
	}

	private static int[] getKSmallestElements(int[] array, int low, int high,
			int k) {
		if (low >= high)
			return new int[0];
		...
		if (pivoteIndex < k)
			return getKSmallestElements(array, pivoteIndex + 1, high, k);
		return getKSmallestElements(array, low, pivoteIndex - 1, k);
	}

	public static void main(String[] args) {
		int[] array = { 56, -45, 0, 78, -3, 555, 4, 10 };
		System.out.print("\nBubble: " + Arrays.toString(bubble(array)));

		System.out.print("\nInsertion sort: "
				+ Arrays.toString(insertion(array)));

		System.out.print("\nMerge Sort:" + Arrays.toString(mergeSort(array)));

		System.out.print("\nQuickSort:" + Arrays.toString(quickSort(array)));

		System.out.print("\nKth smallest element : " + quickSelect(array, 0));
		// wrong op for k=7????????????

		int[] kSmallest = getKSmallestElements(array, 6);
		System.out.print("\nK smallest elements : "
				+ Arrays.toString(kSmallest));
		;
	}
