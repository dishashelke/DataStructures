...
public class Operations {
	...
	//**************print path from root to all leaves******************
	public static void printAllRootToLeafPaths(BinaryTree tree) {
		BinaryNode root = tree.getRoot();
        printAllRootToLeafPaths(root, new ArrayList<Integer>());
    }
    
    private static void printAllRootToLeafPaths(BinaryNode current, List<Integer> path) {
    	if(current == null) {
    		return;
    	}
        ...
        if(current.isLeaf()) {                            
         ...
         /* printAllRootToLeafPaths(current.getLeftChild(), new ArrayList<Integer>(path));
            printAllRootToLeafPaths(current.getRightChild(), new ArrayList<Integer>(path));*/
		printAllRootToLeafPaths(current.getLeftChild(), path);
		path.remove(path.size() - 1);//backtrack to parent
		printAllRootToLeafPaths(current.getRightChild(), path);
		path.remove(path.size() - 1);//backtrack to parent
	}
